...
--3.Display the employee name, job, monthly and annual salary with appropriate column titles.
  SELECT ENAME,JOB,SAL AS 'MONTH_SAL',SAL*12 AS 'ANNUAL_SAL' FROM EMP;
...
--13.Display the employees who were hired before their managers.
   SELECT e.ENAME , e.HIREDATE,m.ENAME,m.HIREDATE FROM EMP e,EMP m WHERE e.EMPNO = m.MGR AND e.HIREDATE < m.HIREDATE;
...
--22.In one query, count the number of people in department in 30 who receive a salary and the number of people who receive a commission.
   SELECT COUNT(SAL) AS 'TOTAL_SAL' ,COUNT(COMM) AS 'TOTAL_COMM'FROM EMP WHERE DEPTNO = 30 ;

--23.Compute the average, minimum and maximum salaries of those groups of employees having the job as clerk or manager.
   SELECT AVG(SAL) AS 'AVERAGE', MIN(SAL) AS 'MINIMUM', MAX(SAL) AS 'MAXIMUM'FROM EMP WHERE JOB = 'CLERK' OR JOB ='MANAGER' ;

--24.Display the department numbers of departments which have more than one clerk.
   SELECT DEPTNO FROM EMP WHERE EXISTS (SELECT COUNT(JOB) AS CLERK_COUNT FROM EMP WHERE JOB = 'CLERK' GROUP BY DEPTNO)GROUP BY DEPTNO;

--25.Which employees earn less than 30 persent of the presidents salary.
   SELECT ENAME , SAL FROM EMP WHERE SAL < (SELECT SAL*0.3 AS SAL FROM EMP  WHERE JOB= 'PRESIDENT');
   
--26.	How many employees work in Chicago?

    SELECT	COUNT(ENAME) FROM EMP JOIN DEPT ON LOC='CHICAGO';

--27.	Which employees work in Chicago?

SELECT	ENAME FROM EMP JOIN DEPT ON LOC='CHICAGO';

--28.	List the employees’ names and cities in which they work. Order the list by city.

SELECT	ENAME,LOC FROM EMP E JOIN DEPT D ON E.DEPTNO=D.DEPTNO ORDER BY LOC; 
...
SELECT	DISTINCT E.DEPTNO,DNAME.LOC,SUM(COMM) AS A TOTAL COMM,SUM(SAL) AS TOTALOFEACHDEPT FROM EMP E,DEPT D GROUP BY E.DEPTNO,DNAME,LOC ORDER BY DEPTNO;
...
select sal,ename,(case when sal = any (select sal from emp where 1250>sal) then 'lessthan1250' when sal = any (select sal from emp where 1600<sal) then 'morethan1600' end) as 'status-result'from emp;
...
select	top 1 e.ename, min(sal) as minsalary from EMP egroup by ename order by minsalary;
...
select ename as Employee, sal as Salary, job as Job, hiredate as 'Date of Hire', comm as Commission,deptno as 'Department No',mgr as 'MGR No'from EMP;


--47.	What is the earliest date on which someone was hired, and what is the latest date on which someone was hired.

select e.ename as EarliestDate , c.ename as LatestDate from emp e , emp c where e.empno = c.mgr and c.HIREDATE < e.HIREDATE;
...
select ename,sal,comm from emp e join  dept don loc='NEW YORK'where e.comm > 10;
